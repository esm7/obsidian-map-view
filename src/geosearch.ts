import { request, App, Editor, Notice, EditorSuggest, EditorPosition, TFile, EditorSuggestTriggerInfo, EditorSuggestContext } from 'obsidian';
...
export class GeoSearcher {
	private searchProvider: geosearch.OpenStreetMapProvider | geosearch.GoogleProvider = null;
	...
	async search(query: string): Promise<GeoSearchResult[]> {
		...
		if (this.settings.useGooglePlaces && this.settings.geocodingApiKey) {
			const placesResults = await googlePlacesSearch(query, this.settings);
			...
			let searchResults = await this.searchProvider.search({query: query});
			searchResults = searchResults.slice(0, consts.MAX_EXTERNAL_SEARCH_SUGGESTIONS);
			results.concat(searchResults.map(result => ({
				name: result.label,
...
export async function googlePlacesSearch(query: string, settings: PluginSettings): Promise<GeoSearchResult[]> {
	...
	if (jsonContent && 'results' in jsonContent && jsonContent?.results.length > 0) {
		for (const result of jsonContent.results) {
			...
			if (location && location.lat && location.lng) {
				const geolocation = new leaflet.LatLng(location.lat, location.lng);
				results.push({
