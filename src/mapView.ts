...
export class MapView extends ItemView {
  ...
  constructor(
    ...
    this.app.vault.on("rename", (file, oldPath) =>
      this.updateMarkersWithRelationToFile(oldPath, file, true)
    );
  ...
  isDarkMode(settings: PluginSettings): boolean {
    ...
    if ((this.app.vault as any).getConfig("theme") === "obsidian") return true;
    return false;
  }
  ...
  updateTileLayerByState(newState: MapState) {
    ...
    if (!this.display.tileLayer) {
      ...
      const attribution =
        chosenMapSource.urlLight === DEFAULT_SETTINGS.mapSources[0].urlLight
          ? '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
          : "";
      let revertMap = false;
      let mapSourceUrl = chosenMapSource.urlLight;
      if (isDark) {
        if (chosenMapSource.urlDark) mapSourceUrl = chosenMapSource.urlDark;
        else revertMap = true;
      }
  ...
  async goToMarker(
    ...
    return this.goToFile(marker.file, useCtrlKeyBehavior, async (editor) => {
      await utils.goToEditorLocation(editor, marker.fileLocation, highlight);
    });
  }
  ...
  private async updateMarkersWithRelationToFile(
    ...
    for (let [markerId, fileMarker] of this.display.markers) {
      if (fileMarker.file.path !== fileRemoved) newMarkers.push(fileMarker);
    }
