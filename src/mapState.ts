...
export function areStatesEqual(state1: MapState, state2: MapState) {
	...
	if (state1.mapCenter) {
		...
		const mapCenter1 = new leaflet.LatLng(state1.mapCenter.lat, state1.mapCenter.lng);
		const mapCenter2 = new leaflet.LatLng(state2.mapCenter.lat, state2.mapCenter.lng);
...
export function stateFromParsedUrl(obj: any) {
	return {
		...
		mapCenter: new leaflet.LatLng(parseFloat(obj.centerLat), parseFloat(obj.centerLng)),
		query: obj.query,
