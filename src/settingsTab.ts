import { App, TextComponent, PluginSettingTab, TextAreaComponent, DropdownComponent, Setting } from 'obsidian';

import MapViewPlugin from 'src/main';
import { PluginSettings, UrlParsingRuleType, UrlParsingContentType, DEFAULT_SETTINGS } from 'src/settings';
...
export class SettingsTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.setDesc('The service used for searching for geolocations. To use Google, see details in the plugin documentation. Changes are applied after restart.')
			.addDropdown(component => {component
				.addOption('osm', 'OpenStreetMap')
				.addOption('google', 'Google (API key required)')
				.setValue(this.plugin.settings.searchProvider || DEFAULT_SETTINGS.searchProvider)
			...
			.addText(component => {
				component
					...
					.onChange(async value => {
						...
						await this.plugin.saveSettings();
						component.inputEl.style.borderColor = value ? '' : 'red';
					});
		...
		new Setting(containerEl)
			.setHeading().setName('URL Parsing Rules')
		...
		new Setting(containerEl)
			.addButton(component => component
				.setButtonText('New Parsing Rule')
				.onClick(() => {
					this.plugin.settings.urlParsingRules.push({name: '', regExp: '', preset: false, ruleType: 'latLng'});
		...
		const iconRulesHeading = new Setting(containerEl)
			.setHeading().setName('Marker Icon Rules');
		...
		new Setting(containerEl)
			.addButton(component => component
				.setButtonText('New Icon Rule')
				.onClick(() => {
					this.plugin.settings.markerIconRules.push({ruleName: '', preset: false, iconDetails: {prefix: 'fas'}});
	...
	refreshUrlParsingRules(containerEl: HTMLElement) {
		...
		for (const setting of parsingRules) {
			...
			let adjustToRuleType = (ruleType: UrlParsingRuleType) => {
				text.setPlaceholder(ruleType === 'fetch' ? 'Regex with 1 capture group' : 'Regex with 2 capture groups');
				if (line2)
					line2.style.display = ruleType === 'fetch' ? 'block' : 'none';
			}
			const controls = new Setting(line1)
				.addText(component => {component
					.setPlaceholder('Name')
					.setValue(setting.name)
					.onChange(async (value: string) => {
			...
			text.setValue(setting.regExp)
				.onChange(async (value: string) => {
					setting.regExp = value;
					await this.plugin.saveSettings();
				});
			controls.addDropdown(component => component
			...
			new TextComponent(line2).setPlaceholder('Regex with 1-2 capture groups')
		...
		const createRules = () => {
			...
			for (const rule of rules) {
				...
				const updateIconAndJson = () => {
					...
					const compiledIcon = getIconFromOptions(Object.assign({},
						rules.find(element => element.ruleName === 'default').iconDetails,
						rule.iconDetails));
			...
			.addTextArea(component => {component
				...
				.onChange(async value => {
					try {
						...
						this.plugin.settings.markerIconRules = newMarkerIcons;
						await this.plugin.saveSettings();
						createRules();
					} catch (e) {
					}
