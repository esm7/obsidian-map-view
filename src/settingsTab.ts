...
export class SettingsTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "Number of total lines to show in the snippet displayed for inline geolocation notes."
      )
      .addSlider((slider) => {
        slider
          .setLimits(0, 12, 1)
          .setDynamicTooltip()
          .setValue(
            this.plugin.settings.snippetLines ?? DEFAULT_SETTINGS.snippetLines
          )
          .onChange(async (value: number) => {
  ...
  refreshUrlParsingRules(containerEl: HTMLElement) {
    ...
    for (const defaultSetting of DEFAULT_SETTINGS.urlParsingRules)
      if (
        parsingRules.findIndex((rule) => rule.name === defaultSetting.name) ===
        -1
      ) {
  ...
  refreshMarkerIcons(containerEl: HTMLElement) {
    ...
    const createRules = () => {
          ...
          .addText(
            (component) =>
              (component
                ...
                .onChange(async (value) => {
                  if (value) rule.iconDetails.markerColor = value;
        ...
        const updateIconAndJson = () => {
          ...
          if (jsonControl)
            jsonControl.setValue(
              JSON.stringify(this.plugin.settings.markerIconRules, null, 2)
            );
        };
      ...
      const ruleTestSetting = new Setting(containerEl)
        ...
        .addText((component) => {
          component.setPlaceholder("#tagOne #tagTwo").onChange((value) => {
