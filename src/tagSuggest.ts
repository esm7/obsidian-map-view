...
export class TagSuggest extends EditorSuggest<SuggestInfo> {
	...
	onTrigger(cursor: EditorPosition, editor: Editor, file: TFile) : EditorSuggestTriggerInfo | null  {
		...
		if (tagMatch)
			return {
				...
				end: {line: cursor.line, ch: tagMatch.index + tagMatch[0].length},
				query: tagMatch[1]
			};
		return null;
	}
	...
	getSuggestions(context: EditorSuggestContext): SuggestInfo[] {
		...
		const matchingTags = utils.getAllTagNames(this.app).filter(
			 value => value.toLowerCase().includes(tagQuery.toLowerCase()));
		return matchingTags.map(tagName => { return {
			tagName: tagName,
			context: context
		}});
	}
