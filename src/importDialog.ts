import { Modal, App, Editor, TextAreaComponent, ButtonComponent } from 'obsidian';
		...
		if (mainDocument) {
			...
			if (places && places.length > 0) {
				...
				for (const place of places) {
					...
					if (placeCoords && placeCoords.length > 0 && typeof placeCoords === 'string') {
						const coordinates = placeCoords.match(regex.COORDINATES);
						if (coordinates && coordinates.length > 3) {
							result.items.push({name: placeName, lat: parseFloat(coordinates[3]), lng: parseFloat(coordinates[1])})
						}
					}
				}
				return result;
...
function styleImportedList(imported: ImportedFile, template: string) : string {
	...
	for (const item of imported.items) {
		const formattedItem = template.replace(/\{\{name}}/g, item.name)
