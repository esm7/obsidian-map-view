...
export class Query {
	...
	constructor(app: App, queryString: string) {
		...
		if (queryString?.length > 0) {
			this.queryRpn = parser.parse(this.preprocessQueryString(queryString));
		} else
	...
	testMarker(marker: FileMarker): boolean {
		if (this.queryEmpty)
			return true;
		const toBool = (s: string) => { return s === 'true'; }
		const toString = (b: boolean) => { return b ? 'true' : 'false'; }
	...
	testIdentifier(marker: FileMarker, value: string): boolean {
		if (value.startsWith('tag:#')) {
			...
			if (fileCache?.links?.some(
					linkCache => linkCache.link.toLowerCase().includes(query) || linkCache.displayText.toLowerCase().includes(query)))
				return true;
			...
			if (fileMatch) {
				...
				if (linksFrom?.links?.some(
					linkCache => linkCache.link.toLowerCase().includes(marker.file.basename.toLowerCase()) || linkCache.displayText.toLowerCase().includes(marker.file.basename.toLowerCase()))) {
					return true;
				}
...
export class QuerySuggest extends PopoverSuggest<Suggestion> {
	...
	open() {
		...
		this.suggestionsDiv.style.top = this.sourceElement.inputEl.getClientRects()[0].bottom + 'px';
		this.suggestionsDiv.style.left = this.sourceElement.inputEl.getClientRects()[0].left + 'px';
		const keyUp = async() => {
	...
	compareSuggestions(suggestions1: Suggestion[], suggestions2: Suggestion[]) {
		...
		for (const [i, s1] of suggestions1.entries()) {
			...
			if (s1.text != s2.text || s1.textToInsert != s2.textToInsert || s1.append != s2.append
					|| s1.insertAt != s2.insertAt || s1.insertSkip != s2.insertSkip)
				return false;
		}
		return true;
	}
	...
	createSuggestions() : Suggestion[] {
		...
		if (tagMatch) {
			...
			for (const tagName of allTagNames) {
				toReturn.push({
					text: tagName, textToInsert: `tag:${tagName} `, insertAt: tagMatch.index,
			...
			return [
				...
				{text: 'linkedto:', textToInsert: 'linkedto:""', cursorOffset: -1},
				{text: 'linkedfrom:', textToInsert: 'linkedfrom:""', cursorOffset: -1},
	...
	createPathSuggestions(pathMatch: RegExpMatchArray, operator: string) : Suggestion[] {
		...
		for (const pathName of allPathNames) {
			toReturn.push({
				text: pathName, textToInsert: `${operator}:"${pathName}" `, insertAt: pathMatch.index,
	...
	selectSuggestion(suggestion: Suggestion, event: MouseEvent | KeyboardEvent) {
		// We don't use it, but need it here to inherit from QuerySuggest
		if (!suggestion.group) {
			const insertAt = suggestion.insertAt != null ? suggestion.insertAt : this.sourceElement.inputEl.selectionStart;
			...
			const newText = currentText.substring(0, insertAt) + addedText + currentText.substring(insertAt + insertSkip);
			this.sourceElement.setValue(newText);
			this.sourceElement.inputEl.selectionEnd = this.sourceElement.inputEl.selectionStart = insertAt + addedText.length + (suggestion?.cursorOffset ?? 0);
	...
	getAllPathNames(search: string) : string[] {
		...
		for (const file of allFiles) {
			if (!search || search && file.path.toLowerCase().includes(search.toLowerCase()))
